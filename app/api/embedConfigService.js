 ...
 */
async function getEmbedInfo(reportName) {

    // Get the Report Embed details
    try {

        // Get report details and embed token
        const embedConfig = config.getPowerBIConfig(reportName);
        const embedParams = await getEmbedParamsForSingleReport(embedConfig.workspaceId, embedConfig.reportId);
