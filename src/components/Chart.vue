<template>
  <div class="line_chart">
    <div id="myChart"  v-show="!flag"></div>
    ...
    options() {
      return {
        tooltip: {
            ...
            //   newParams.forEach((p) => {
            //     const cont = '<p>' + p.marker + ' ' + p.seriesName + ': ' + p.value + '</p>';
            ...
            formatter: function (params) {
              ...
              newParams.sort((a,b) => {return b.value - a.value});
              newParams.forEach((p) => {
                const cont = '<p>'   + colorSpan(p.color) + ' ' + p.seriesName + ': ' + p.data + '</p>'
                 rez += cont;
                tooltipString.push(cont);
              });
              return rez;
